...
const deviceColumn: ColumnDef<Device>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => <EditDevice
      key={row.id}
      device={row.original}
      trigger={<PrimaryButton text='edit' size='small' />}
      close={<div className='inline-flex'></div>}
    />,
  },
];
...
function DeviceTableBody({ table }: DeviceTableElementProps) {
  ...
  return (
    ...
    {rows.map((row) => (
      <tr
        key={row.id}
        className='hover:bg-gray-100 hover:font-medium transition-all h-8 border-b'
      >
...
function SearchInput() {
  ...
  return (
    <div
      className={`flex flex-row justify-center items-center w-full h-10 border bg-surface rounded-full pr-1.5 ${focusBorder}`}
    >
